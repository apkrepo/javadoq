...
/**
 * <p>A JJTree visitor to collect super Java types extended in a source file.</p>
 * 
 ...
 */
public class SuperTypeVisitor implements JJTreeParserVisitor
{
	/**
	 * <p>The source file where this visitor is collecting super types.</p>
	 * @since 1.0
	 */
	public final SourceFile file;

	/**
	 * <p>Constructs a {@link SuperTypeVisitor}.</p>
	 * @param file The source file where this visitor is collecting super types.
	 * @since 1.0
	 */
	...
	public Object visit(ASTNewType node, Object data) {
		JavaType t;
		if (stack.isEmpty()) {
			t = file.pckg.types.get(node.name);
		} else {
	...
	public Object visit(ASTSuperList node, Object data) {
		data = node.childrenAccept(this, data);
		JavaType current = stack.get(stack.size() - 1);
		for (String s : node.list) {
			JavaName name = new JavaName(s);
			JavaType t = null;
			if (current instanceof JavaNestedType) {
				t = ((JavaNestedType)current).outer.findType(name);
			}
			if (t == null) {
				t = current.file.findType(name);
			}
			if (t != null) {
				current.supers.put(t.qname.text, t);
			}
		}
		return data;
	}
	...
	public Object visit(SimpleNode node, Object data) {
		...
		} else if (node instanceof ASTSuperList) {
			return visit((ASTSuperList)node, data);
		}
		return data;
	}
